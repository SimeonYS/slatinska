...
class SlatinskaPipeline:

    # Database setup
    conn = sqlite3.connect('slatinska.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `slatinska`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM slatinska WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO slatinska (date, title, link, content)"
